...
class MelcloudAtwDevice {
				...
				.then(function handleSetDeviceResponse(response) {
					if (!response) {
						...
						switch (deviceOption) {
							...
							case commonDefines.AtwDeviceOptions.OperationModeZone2:
								if (gthis.hasZone2) {
									gthis.operationModeZone2 = responseData.OperationModeZone2;
								}
								break;
							case commonDefines.AtwDeviceOptions.SetTankWaterTemperature:
							...
							case commonDefines.AtwDeviceOptions.SetTemperatureZone2:
								if (gthis.hasZone2) {
									gthis.setTemperatureZone2 = responseData.SetTemperatureZone2;
								}
								break;
							case commonDefines.AtwDeviceOptions.SetHeatFlowTemperatureZone1:
								if (gthis.canHeat) {
								...
								break;
							case commonDefines.AtwDeviceOptions.SetHeatFlowTemperatureZone2:
								if (gthis.canHeat && gthis.hasZone2) {
									gthis.setHeatFlowTemperatureZone2 = responseData.SetHeatFlowTemperatureZone2;
								}
								break;
							case commonDefines.AtwDeviceOptions.SetCoolFlowTemperatureZone1:
								if (gthis.canCool) {
								...
								break;
							case commonDefines.AtwDeviceOptions.SetCoolFlowTemperatureZone2:
								if (gthat.canCool && gthis.hasZone2) {
									gthis.setCoolFlowTemperatureZone2 = responseData.SetCoolFlowTemperatureZone2;
								}
								break;
							default:
								break;
	...
	async getPowerConsumptionReport(isCumulatedReport = false) {
		return new Promise((resolve, reject) => {
			gthat.log.debug(`Getting power consumption report for ${this.id} (${this.name})`);
	...
	async toggleTimerState(enableTimer) {
		return new Promise((resolve, reject) => {
			gthat.log.debug(`${enableTimer ? `Enabling` : `Disabling`} timer for ${this.id} (${this.name})`);
