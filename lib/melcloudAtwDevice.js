...
class MelcloudAtwDevice {
	constructor(that) {
		...
		// Control
		this.power = false;
		this.operationMode = commonDefines.AtwDeviceOperationModes.UNDEF.value;
	}
	...
	async CreateAndSave() {
		...
		await gthat.extendObjectAsync(controlPrefix + commonDefines.AtwDeviceStateIDs.Mode, {
			type: "state",
			common: {
				name: "Operation mode",
				type: "number",
	...
	async UpdateDeviceData(deviceOption) {
		...
		await gthat.setStateChangedAsync(infoPrefix + commonDefines.AtwDeviceStateIDs.RoomTemperatureZone2, this.roomTemperatureZone2, true);
		...
		await gthat.setStateChangedAsync(infoPrefix + commonDefines.AtwDeviceStateIDs.FlowTemperatureZone2, this.flowTemperatureZone2, true);
		...
		await gthat.setStateChangedAsync(infoPrefix + commonDefines.AtwDeviceStateIDs.ReturnTemperatureZone2, this.ReturnTemperatureZone2, true);
		...
		switch (deviceOption) {
			...
			default:
				await gthat.setStateChangedAsync(controlPrefix + commonDefines.AtwDeviceStateIDs.Power, this.power, true);
				await gthat.setStateChangedAsync(controlPrefix + commonDefines.AtwDeviceStateIDs.Mode, this.operationMode, true);
				break;
		}
		//#endregion
	...
	setDevice(deviceOption, value, gthis) {
		if (gthis.currentDeviceSetRequests < 1) {
			...
			else if (deviceOption == commonDefines.AtwDeviceOptions.OperationMode) {
				switch (value) {
					case commonDefines.AtwDeviceOperationModes.HEATTHERMOSTAT:
						r.OperationMode = commonDefines.AtwDeviceOperationModes.HEATTHERMOSTAT.value;
						r.EffectiveFlags = commonDefines.AtwDeviceOperationModes.HEATTHERMOSTAT.effectiveFlags;
						break;
					case commonDefines.AtwDeviceOperationModes.HEATFLOW:
						r.OperationMode = commonDefines.AtwDeviceOperationModes.HEATFLOW.value;
						r.EffectiveFlags = commonDefines.AtwDeviceOperationModes.HEATFLOW.effectiveFlags;
						break;
					case commonDefines.AtwDeviceOperationModes.CURVE:
						r.OperationMode = commonDefines.AtwDeviceOperationModes.CURVE.value;
						r.EffectiveFlags = commonDefines.AtwDeviceOperationModes.CURVE.effectiveFlags;
						break;
					case commonDefines.AtwDeviceOperationModes.COOLTHERMOSTAT:
						r.OperationMode = commonDefines.AtwDeviceOperationModes.COOLTHERMOSTAT.value;
						r.EffectiveFlags = commonDefines.AtwDeviceOperationModes.COOLTHERMOSTAT.effectiveFlags;
						break;
					case commonDefines.AtwDeviceOperationModes.COOLFLOW:
						r.OperationMode = commonDefines.AtwDeviceOperationModes.COOLFLOW.value;
						r.EffectiveFlags = commonDefines.AtwDeviceOperationModes.COOLFLOW.effectiveFlags;
						break;
					default:
						gthat.log.error("Unsupported value for device option - please report this to the developer!");
						return;
				...
				if (!response) {
					...
					switch (deviceOption) {
						case commonDefines.AtwDeviceOptions.PowerState:
							...
							break;
						case commonDefines.AtwDeviceOptions.OperationMode:
