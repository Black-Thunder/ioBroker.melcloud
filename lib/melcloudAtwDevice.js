		...
		if (gthis.currentDeviceSetRequests < 1) {
			...
			if (deviceOption == commonDefines.AtwDeviceOptions.PowerState) {
				...
				switch (value) {
					case commonDefines.AtwDeviceZoneOperationModes.HEATTHERMOSTAT:
						modifiedAirInfo.OperationMode = commonDefines.AtwDeviceZoneOperationModes.HEATTHERMOSTAT.value;
						break;
					case commonDefines.AtwDeviceZoneOperationModes.HEATFLOW:
						modifiedAirInfo.OperationMode = commonDefines.AtwDeviceZoneOperationModes.HEATFLOW.value;
						break;
					case commonDefines.AtwDeviceZoneOperationModes.CURVE:
						modifiedAirInfo.OperationMode = commonDefines.AtwDeviceZoneOperationModes.CURVE.value;
						break;
					case commonDefines.AtwDeviceZoneOperationModes.COOLTHERMOSTAT:
						modifiedAirInfo.OperationMode = commonDefines.AtwDeviceZoneOperationModes.COOLTHERMOSTAT.value;
						break;
					case commonDefines.AtwDeviceZoneOperationModes.COOLFLOW:
						modifiedAirInfo.OperationMode = commonDefines.AtwDeviceZoneOperationModes.COOLFLOW.value;
						break;
					default:
						gthat.log.error("setDevice(): Unsupported value for device option - please report this to the developer!");
						return;
			...
			Axios.post(url, body, {
				...
				if (!response) {
					...
					if (responseData.HasError != undefined) {
						gthis.deviceHasError = responseData.HasError;
					}

					if (responseData.ErrorMessages != undefined) {
						gthis.errorMessages = responseData.ErrorMessages;
					}
