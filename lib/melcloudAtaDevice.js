...
class MelcloudAtaDevice {
	...
	async buildPowerConsumptionReportRequestBody() {
		...
		const startDateObj = await gthat.getStateAsync(`${commonDefines.AdapterDatapointIDs.Devices}.${this.id}.${commonDefines.AdapterDatapointIDs.Reports}.${commonDefines.AtaDeviceStateIDs.PowerConsumptionReportStartDate}`);
		let startDate = (startDateObj == null || startDateObj.val == null) ? "" : startDateObj.val;
		const endDateObj = await gthat.getStateAsync(`${commonDefines.AdapterDatapointIDs.Devices}.${this.id}.${commonDefines.AdapterDatapointIDs.Reports}.${commonDefines.AtaDeviceStateIDs.PowerConsumptionReportEndDate}`);
