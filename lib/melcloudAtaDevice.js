...
class MelcloudAtaDevice {
	...
	async CreateAndSave() {
		...
		await gthat.extendObjectAsync(reportsPrefix + commonDefines.AtaDeviceStateIDs.PowerConsumptionReportStartDate, {
			type: "state",
			common: {
		...
		await gthat.extendObjectAsync(reportsPrefix + commonDefines.AtaDeviceStateIDs.PowerConsumptionReportEndDate, {
			type: "state",
			common: {
		...
		operationModes.forEach(mode => {
			...
			gthat.extendObjectAsync(lastReportDataPrefix + commonDefines.AtaDeviceStateIDs.TotalPowerConsumptionPrefix + mode, {
				type: "state",
				common: {
		...
		gthat.extendObjectAsync(lastReportDataPrefix + commonDefines.AtaDeviceStateIDs.TotalPowerConsumptionPrefix, {
			type: "state",
			common: {
		...
		await gthat.extendObjectAsync(lastReportDataPrefix + commonDefines.AtaDeviceStateIDs.TotalReportedMinutes, {
			type: "state",
			common: {
		...
		await gthat.extendObjectAsync(lastReportDataPrefix + commonDefines.AtaDeviceStateIDs.RawPowerConsumptionData, {
			type: "state",
			common: {
	...
	async UpdateReportData() {
		...
		await gthat.setStateChangedAsync(reportsPrefix + commonDefines.AtaDeviceStateIDs.PowerConsumptionReportStartDate, this.powerConsumptionReportStartDate, true);
		await gthat.setStateChangedAsync(reportsPrefix + commonDefines.AtaDeviceStateIDs.PowerConsumptionReportEndDate, this.powerConsumptionReportEndDate, true);
		...
		if (this.canCool) {
			await gthat.setStateChangedAsync(lastReportDataPrefix + commonDefines.AtaDeviceStateIDs.TotalPowerConsumptionPrefix + commonDefines.AtaDeviceOperationModes.COOL.id, commonDefines.roundValue(this.totalPowerConsumptionCooling, 3), true);
			totalConsumption += this.totalPowerConsumptionCooling;
		}

		if (this.canHeat) {
			await gthat.setStateChangedAsync(lastReportDataPrefix + commonDefines.AtaDeviceStateIDs.TotalPowerConsumptionPrefix + commonDefines.AtaDeviceOperationModes.HEAT.id, commonDefines.roundValue(this.totalPowerConsumptionHeating, 3), true);
			totalConsumption += this.totalPowerConsumptionHeating;
		}

		await gthat.setStateChangedAsync(lastReportDataPrefix + commonDefines.AtaDeviceStateIDs.TotalPowerConsumptionPrefix + commonDefines.AtaDeviceOperationModes.AUTO.id, commonDefines.roundValue(this.totalPowerConsumptionAuto, 3), true);
		totalConsumption += this.totalPowerConsumptionAuto;

		if (this.canDry) {
			await gthat.setStateChangedAsync(lastReportDataPrefix + commonDefines.AtaDeviceStateIDs.TotalPowerConsumptionPrefix + commonDefines.AtaDeviceOperationModes.DRY.id, commonDefines.roundValue(this.totalPowerConsumptionDry, 3), true);
			totalConsumption += this.totalPowerConsumptionDry;
		}

		await gthat.setStateChangedAsync(lastReportDataPrefix + commonDefines.AtaDeviceStateIDs.TotalPowerConsumptionPrefix + commonDefines.AtaDeviceOperationModes.VENT.id, commonDefines.roundValue(this.totalPowerConsumptionVent, 3), true);
		totalConsumption += this.totalPowerConsumptionVent;

		await gthat.setStateChangedAsync(lastReportDataPrefix + commonDefines.AtaDeviceStateIDs.TotalPowerConsumptionPrefix, commonDefines.roundValue(totalConsumption, 3), true);

		await gthat.setStateChangedAsync(lastReportDataPrefix + commonDefines.AtaDeviceStateIDs.TotalReportedMinutes, this.totalPowerConsumptionMinutes, true);
		await gthat.setStateChangedAsync(lastReportDataPrefix + commonDefines.AtaDeviceStateIDs.RawPowerConsumptionData, JSON.stringify(this.rawPowerConsumptionReportData), true);
	...
	async buildPowerConsumptionReportRequestBody(isCumulatedReport = false) {
		...
		const startStateId = isCumulatedReport ? `${commonDefines.AdapterDatapointIDs.Reports}.${commonDefines.AtaDeviceStateIDs.PowerConsumptionReportStartDate}` : `${commonDefines.AdapterDatapointIDs.Devices}.${this.id}.${commonDefines.AdapterDatapointIDs.Reports}.${commonDefines.AtaDeviceStateIDs.PowerConsumptionReportStartDate}`;
		...
		const endStateId = isCumulatedReport ? `${commonDefines.AdapterDatapointIDs.Reports}.${commonDefines.AtaDeviceStateIDs.PowerConsumptionReportEndDate}` : `${commonDefines.AdapterDatapointIDs.Devices}.${this.id}.${commonDefines.AdapterDatapointIDs.Reports}.${commonDefines.AtaDeviceStateIDs.PowerConsumptionReportEndDate}`;
