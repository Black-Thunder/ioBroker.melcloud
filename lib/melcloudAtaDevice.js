		...
		switch (deviceOption) {
			case commonDefines.AtaDeviceOptions.FanSpeed:
				if (value > gthis.numberOfFanSpeeds) {
					gthat.log.warn(
						`Fan speed limited to ${gthis.numberOfFanSpeeds} because device can't handle more than that!`,
					);
			...
			(async () => {
				...
				try {
					...
					try {
						...
						gthat.log.debug(`Response from cloud: ${JSON.stringify(setResponseData)}`);
