...
const contextKeyInvalidationTimer = 12 * 60 * 1000; // time (in ms) after which a new context key should be obtained
...
class MelcloudPlatform {
			...
			.then(function handleLoginResponse(response) {
				if (!response) {
					...
					if (responseData.LoginData == null) {
						...
						gthis.isContextKeyValid = true;

						contextKeyInvalidationTimeout = setTimeout(async function invalidateContextKey() {
							gthis.isContextKeyValid = false;
							gthat.log.debug(`Context key invalidated. Getting a new one for the next polling interval.`);
	...
	startPolling() {
		...
		let jobInterval = gthat.config.pollingInterval * 60000 + Math.floor(Math.random() * 1000); // polling interval in milliseconds plus some random amount between 0 and 1000ms

		pollingJob = setTimeout(async function updateData() {
			jobInterval = gthat.config.pollingInterval * 60000 + Math.floor(Math.random() * 1000); // slightly change interval every run

			if (gthis.isContextKeyValid) {
				gthat.log.debug("Next polling interval: Using the same (old) context key.");
				gthis.GetDevices(gthis.CreateAndSaveDevices);
			}
			else {
				gthat.log.debug("Next polling interval: Getting a new context key.");
