...
class MelcloudPlatform {
	...
	async GetContextKey(callback, callback2) {
		...
		form.append("AppVersion", "1.26.2.0");
		...
		for (let d = 0; d < devices.length; d++) {
			...
			switch (deviceType) {
				...
				case commonDefines.DeviceTypes.EnergyRecoveryVentilation:
					gthis.CreateErvDevice(building, deviceJson, foundDevices);
					break;
				default:
	...
	CreateErvDevice(building, deviceJson, foundDevices) {
		const newDevice = new MelCloudErvDevice.MelCloudDevice(gthat);
		...
		gthat.log.debug(`Got ERV device from cloud: ${deviceJson.DeviceID} (${deviceJson.DeviceName})`);
		foundDevices.push(newDevice);
		gthat.deviceObjects.push(newDevice);
	}
