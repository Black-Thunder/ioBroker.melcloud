...
class MelcloudErvDevice extends MelcloudBaseDevice {
	...
	setDevice(deviceOption, value, retryCount = 0) {
			...
			if (modifiedAirInfo == null) {
				...
				if (retryCount < 1) {
					this.adapter.log.warn(
						`setDevice(): modifiedAirInfo is not filled. Refreshing device info and retrying once...`,
					);
					this.getDeviceInfo(
						(option, nextValue) => this.setDevice(option, nextValue, retryCount + 1),
						deviceOption,
						value,
					);
					return;
				...
				this.adapter.log.error(
					`setDevice(): modifiedAirInfo is still not filled after refresh. Aborting request for '${deviceOption.id}'.`,
				);
