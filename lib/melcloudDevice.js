...
class MelcloudDevice {
	...
	async UpdateReportData() {
		...
		this.powerConsumptionReportMonths.forEach(month => {
			...
			gthat.setStateChangedAsync(reportsPrefix + commonDefines.AdapterStateIDs.TotalPowerConsumptionPrefix + commonDefines.DeviceOperationModes.COOL.id + monthName, this.monthlyPowerConsumptionCooling[counter], true);
			gthat.setStateChangedAsync(reportsPrefix + commonDefines.AdapterStateIDs.TotalPowerConsumptionPrefix + commonDefines.DeviceOperationModes.HEAT.id + monthName, this.monthlyPowerConsumptionHeating[counter], true);
			gthat.setStateChangedAsync(reportsPrefix + commonDefines.AdapterStateIDs.TotalPowerConsumptionPrefix + commonDefines.DeviceOperationModes.AUTO.id + monthName, this.monthlyPowerConsumptionAuto[counter], true);
			gthat.setStateChangedAsync(reportsPrefix + commonDefines.AdapterStateIDs.TotalPowerConsumptionPrefix + commonDefines.DeviceOperationModes.DRY.id + monthName, this.monthlyPowerConsumptionDry[counter], true);
			gthat.setStateChangedAsync(reportsPrefix + commonDefines.AdapterStateIDs.TotalPowerConsumptionPrefix + commonDefines.DeviceOperationModes.VENT.id + monthName, this.monthlyPowerConsumptionVent[counter], true);
			counter++;
		});

		await gthat.setStateChangedAsync(reportsPrefix + commonDefines.AdapterStateIDs.TotalPowerConsumptionPrefix + commonDefines.DeviceOperationModes.COOL.id, this.totalPowerConsumptionCooling, true);
		await gthat.setStateChangedAsync(reportsPrefix + commonDefines.AdapterStateIDs.TotalPowerConsumptionPrefix + commonDefines.DeviceOperationModes.HEAT.id, this.totalPowerConsumptionHeating, true);
		await gthat.setStateChangedAsync(reportsPrefix + commonDefines.AdapterStateIDs.TotalPowerConsumptionPrefix + commonDefines.DeviceOperationModes.AUTO.id, this.totalPowerConsumptionAuto, true);
		await gthat.setStateChangedAsync(reportsPrefix + commonDefines.AdapterStateIDs.TotalPowerConsumptionPrefix + commonDefines.DeviceOperationModes.DRY.id, this.totalPowerConsumptionDry, true);
		await gthat.setStateChangedAsync(reportsPrefix + commonDefines.AdapterStateIDs.TotalPowerConsumptionPrefix + commonDefines.DeviceOperationModes.VENT.id, this.totalPowerConsumptionVent, true);
	...
	async getPowerConsumptionReport() {
		...
		request({
			...
			if (err) {
				...
				gthis.totalPowerConsumptionCooling = commonDefines.roundValue(parsedReport.TotalCoolingConsumed, 3);
				gthis.totalPowerConsumptionHeating = commonDefines.roundValue(parsedReport.TotalHeatingConsumed, 3);
				gthis.totalPowerConsumptionAuto = commonDefines.roundValue(parsedReport.TotalAutoConsumed, 3);
				gthis.totalPowerConsumptionDry = commonDefines.roundValue(parsedReport.TotalDryConsumed, 3);
				gthis.totalPowerConsumptionVent = commonDefines.roundValue(parsedReport.TotalFanConsumed, 3);
