...
class MelcloudDevice {
	...
	async CreateAndSave() {
		...
		await gthat.setObjectNotExistsAsync(reportsPrefix + commonDefines.AdapterStateIDs.TotalPowerConsumptionCooling, {
			type: "state",
			common: {
				name: "Total power consumption cooling",
				type: "number",
				role: "value",
				min: 0,
				read: true,
				...
				unit: "kWh",
				def: "0",
				desc: "Total power consumption for mode 'COOL'"
			},
			native: {}
		});
		//// END REPORTS
	...
	async UpdateReportData() {
		...
		await gthat.setStateChangedAsync(reportsPrefix + commonDefines.AdapterStateIDs.TotalPowerConsumptionCooling, this.totalPowerConsumptionCooling, true);
	...
	async buildPowerConsumptionReportRequestBody() {
		...
		const startDate = await gthat.getStateAsync(commonDefines.AdapterDatapointIDs.Devices + "." + this.id + "." + commonDefines.AdapterDatapointIDs.Reports + "." + commonDefines.AdapterStateIDs.ReportStartDate);
		const endDate = await gthat.getStateAsync(commonDefines.AdapterDatapointIDs.Devices + "." + this.id + "." + commonDefines.AdapterDatapointIDs.Reports + "." + commonDefines.AdapterStateIDs.ReportEndDate);

		const d = new Date();
		if(!startDate || !startDate.val || startDate.val == "") {
		...
		const parsedStartDate = startDate.val.split("-");
		...
		if(!endDate || !endDate.val || endDate.val == "") {
		...
		const parsedEndDate = endDate.val.split("-");
		...
		requestBody.FromDate = startDate.val + "T00:00:00";
		requestBody.ToDate = endDate.val + "T00:00:00";
