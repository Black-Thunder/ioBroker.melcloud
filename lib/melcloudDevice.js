...
class MelcloudDevice {
	...
	async UpdateDeviceData() {
		...
		const cache = gthat.getStateValueCache();

		if (cache[gthat.namespace + "." + controlPrefix + commonDefines.AdapterStateIDs.Power] != this.power) {
			await gthat.setStateChangedAsync(controlPrefix + commonDefines.AdapterStateIDs.Power, this.power, true);
		}

		if (cache[gthat.namespace + "." + controlPrefix + commonDefines.AdapterStateIDs.Mode] != this.operationMode) {
			await gthat.setStateChangedAsync(controlPrefix + commonDefines.AdapterStateIDs.Mode, this.operationMode, true);
		}

		if (cache[gthat.namespace + "." + controlPrefix + commonDefines.AdapterStateIDs.TargetTemp] != this.targetTemp) {
			await gthat.setStateChangedAsync(controlPrefix + commonDefines.AdapterStateIDs.TargetTemp, this.targetTemp, true);
		}

		if (cache[gthat.namespace + "." + controlPrefix + commonDefines.AdapterStateIDs.FanSpeedManual] != this.fanSpeed) {
			await gthat.setStateChangedAsync(controlPrefix + commonDefines.AdapterStateIDs.FanSpeedManual, this.fanSpeed, true);
		}

		if (cache[gthat.namespace + "." + controlPrefix + commonDefines.AdapterStateIDs.VaneVerticalDirection] != this.vaneVerticalDirection) {
		...
		if (cache[gthat.namespace + "." + controlPrefix + commonDefines.AdapterStateIDs.VaneHorizontalDirection] != this.vaneHorizontalDirection) {
